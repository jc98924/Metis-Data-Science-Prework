...
Baye's Theorem is denoted by the formula:

P(A|B) = (P(B|A)*P(A)) / P(B)

Because we are given the incidence of both indentical and fraternal twins in the population, we can calculate total percent of twins in the population and the spread between fraternal and identical twins.

```
fraternal = 1/125
identical = 1/300
total_twins = (5/1500) + (12/1500)
percent_identical = identical/total_twins
percent_fraternal = fraternal/total_twins

```
In this scenario:
**P(A)** = chance_indentical
**P(B|A)** = 50% (chance of being male)
**P(B)** = chance of being a male twins

We can plug these values into our formula listed above.

```
percent_identical * .5/(.5*percent_identical+ .25 * percent_fraternal)
>> 0.45454545454545464
...
The biggest difference between Frequentist and Bayesian statistic is how probability is interpreted. In frequentist statistics, the value of probability is determined by repeated observances over the long-run. In Bayesian statistics, prior or updated knowledge can influence the conclusion. Although Bayesian statistics can lead to more accurate probable outcomes, this limits the complexity of large scale computations. 
